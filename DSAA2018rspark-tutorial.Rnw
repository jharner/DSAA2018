...
This tutorial covers data science workflows using \proglang{R} as both an analysis engine and as an interface to databases, Spark, etc.\\
\vspace{0.5 cm}
The primary language used in the tutorial is \proglang{R}, but a working knowledge of bash scripts and SQL is useful. You should download and install R if you have not already done so. Go to: \texttt{https://www.r-project.org}\\
\vspace{0.5 cm}
...
\vspace{0.5 cm}
This local repo can then be imported into the computional environment used in this tutorial, which is called \texttt{rspark}.
\end{frame}
...
\vspace{0.5 cm}
The \texttt{rspark} computational environment is available in several GitHub repos depending on whether you want to built the environment from scratch or you you want to download pre-built images. Assuming the latter, go to the rspark-docker repo:\\
