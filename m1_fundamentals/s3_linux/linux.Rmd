...
./start.sh build
```
This will build the images and launch the containers. Login as before. Don't use the `build` option, i.e., simply start the containers by:
```
./start.sh
```
...
The bash script `dockill.sh` in the `rspark` GitHub repo has a series of `docker` commands to control your images, containers, and volumes.
