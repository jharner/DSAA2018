...
We will use the binary executable `psql` in Section 2.4. R scripts associated with Hadoop are run in this Chapter 4.
...
Lines beginning with a `d` indicate a directory, whereas those beginning with a `-` indicate a file. For example, `rspark-notes`, is the directory containing the course notes. Files or directories beginning with a `.` are hidden, e.g., `.bash_history` is a hidden file, whereas `.rstudio` is a hidden directory. There are two special hidden directories: `.` refers to the current directory and `..` is its parent. A common use case for `..` is to type `cd ..` to move up one level in the directory hierarchy.

We often use `.`, current directory, to execute interpreted scripts, e.g., `./start.sh build` to build the Docker containers for rspark from Docker Hub (see Section 1.3.3 below). Here `start` is the name of the bash script with the extension `.sh`. An R script has `.R` as the extension.

The permission string is given by the first 10 characters in each output line. After the first character, we have permissions (file/directory modes) in three sequences of three characters each. For example, for `rspark-notes`: 
...
Each container has an associated Dockerfile, which is a script for building the layers of a Docker image. Images can be built from a base image. For example, `rstudio` is built from `rocker:verse`, an image for running R from RStudio. A Docker container is simply a running instance of an image.
...
Once Hyper-V is installed, go here to get [Docker for the Windows](https://www.docker.com/docker-windows) and follow the directions for installation.  
...
```
You are now ready to start your session. Run:
```
cd rspark-docker
./start.sh build
```
The last command runs the `start.sh` shell script and builds the containers. You must have execute privileges. If not, use `chmod`, e.g., `chmod 755 start.sh`.

It will take awhile the first time or after a pull from GitHub. Eventually, you will reach a line somewhat like: "hive_1      | SLF4J: Actual binding is of type [org.apache.logging.slf4j.Log4jLoggerFactory]"  You will *not* return to the terminal prompt, but the containers are nevertheless running. _IMPORTANT: leave the program running in the terminal, i.e., do NOT use Control-C and do not quit the terminal_.
...
./start.sh build
```
This will build the images and launch the containers. Login as before. From then on don't use the `build` option, i.e., simply start the containers by:
```
./start.sh
```
...
cd home/rstudio
```
where "rspark-docker_rstudio_1" is the container `NAME`. You can get a list of running containers by typing `docker ps` into your local terminal or `docker ps -a` to list both the running and stopped containers.

At this point you are in the `bash` shell within the `rstudio` container. You can now run a command-line version of R by simply typing `R`, a binary executable, at the `#` prompt. This will give you an R console in your terminal with `>` as its prompt. However, `ssh`ing into a container is generally done for debugging in creating the container's Dockerfile---not for running application programs.
